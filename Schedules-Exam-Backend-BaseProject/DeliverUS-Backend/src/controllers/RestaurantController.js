...
const showWithActiveProducts = async function (req, res) {
  try {
    ...
    // 1. Buscamos el restaurante con todos sus productos y horarios
    const restaurant = await Restaurant.findByPk(req.params.restaurantId, {
      include: {
        model: Schedule,
        as: 'schedule',
    ...
    // 2. Filtramos en memoria para evitar el error de Sequelize
    restaurant.dataValues.products = restaurant.products.filter(product => {
